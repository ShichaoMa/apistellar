...
class TestErrorHook(object):

    def test_with_code_message_and_extra_debug(self):
        hook = ErrorHook()
        app = namedtuple("App", "debug")
        app.debug = True
        content = hook.on_error(AssertionError(
            [123, "error", {"id": 1111}]), app).content
    ...
    def test_with_code_message_and_extra_not_debug(self):
        ...
        app = namedtuple("App", "debug")
        app.debug = False
        content = hook.on_error(
            AssertionError([123, "error", {"id": 1111}]), app).content
        data = json.loads(content)
        assert data["code"] == 123
        assert data["message"] == "error"
        assert data["extra"] == {"id": 1111}
    ...
    def test_message_and_extra(self):
        hook = ErrorHook()
        app = namedtuple("App", "debug")
        app.debug = False
